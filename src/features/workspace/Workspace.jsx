...
function LoadingWorkspace() {
  return (
    <main className="pb-boot">
      <div className="pb-boot-mark">PB</div>
      <strong>Préparation de ton espace terrain…</strong>
      <span>Synchronisation du portefeuille pharmacie.</span>
    </main>
  );
}
...
export default function Workspace({ session }) {
  ...
  const searchResults = useMemo(() => {
    ...
    if (!query) return [];
    return state.pharmacies.filter((pharmacy) => [pharmacy.name, pharmacy.city, pharmacy.postal_code, pharmacy.groupement]
      .filter(Boolean)
      .join(' ')
      .toLowerCase()
      .includes(query))
  ...
  function openAccount(pharmacyId) {
    setSelectedAccountId(pharmacyId);
  ...
  if (activeTab === 'dashboard') {
    content = <Dashboard onCompleteTask={completeTask} onNavigate={setActiveTab} onOpenAccount={openAccount} state={state} />;
  } else if (activeTab === 'accounts') {
    content = <AccountsView onAddRelation={addRelation} onCreateTask={createTask} onSelectAccount={selectAccount} onUpdateRelation={updateRelation} selectedAccountId={selectedAccountId} state={state} />;
  } else if (activeTab === 'pipeline') {
    content = <PipelineView onOpenAccount={openAccount} state={state} />;
